...
class MovingAverageCrossoverStrategy(bt.Strategy):
    params = (
        ...
        ('rule_201_threshold', -99.99),  # -10% threshold with 0.01% buffer
    ...
    def can_buy(self, data, current_date):
        ...
        if rule_201_blocked:
            ...
            logging.warning(f"Skipping {symbol} due to active Rule 201 restriction")
    ...
    def is_buy_signal(self, data):
        # Ensure there are at least some data points
        if len(data) < 3:
            return False
        ...
        # Check if price is above its 5-day SMA (simple momentum check)

        # Using the 0.70-0.80 range where win rates dramatically improve
        primary_signal = current_prob >= 0.68
...
def main():
    ...
    # Initialize file_paths as an empty list by default
    file_paths = []
    ...
    if args.BrokerTest > 0:
        ...
        # If neither option is specified, default to all files
        logger.info("Neither BrokerTest nor RunStocks specified, defaulting to all files")
        file_paths = [os.path.join('Data/RFpredictions', f) for f in os.listdir('Data/RFpredictions') if f.endswith('.parquet')]
        file_paths.sort(key=lambda x: x.lower())

    # Check if we found any files
    if not file_paths:
        logger.error("No .parquet files found in Data/RFpredictions directory")
        print("No .parquet files found in Data/RFpredictions directory")
        return



    logger.info(f"Found {len(file_paths)} .parquet files to process")
