...
class MyStrategy(bt.Strategy):
    ...
    def notify_order(self, order):
        """Updated order notification handler with bracket failure detection and recovery"""
        ...
        if order.status in [order.Completed]:
            ...
            if order.isbuy():
                # Process buy order normally
                self.handle_buy_order(order)

                # Check if this was a main order in a bracket that needs protection
                if hasattr(order, 'parent') and order.parent is None:  # It's a parent order (no parent of its own)
                    ...
                    if len(child_orders) < 2:  # Should have at least stop and limit children
                        logging.warning(f"Bracket order children missing for {symbol}. Creating protection orders.")
                        ...
                        for child in child_orders:
                            if child.status in [order.Canceled, order.Rejected]:
                                logging.warning(f"Child order failed for {symbol}. Creating protection orders.")
                                self.handle_bracket_failure(symbol, order.executed.size, order.executed.price)
                                break
            ...
            logging.info(f"Order for {symbol} {order.getstatusname()}")
        ...
        # Handle failed orders
            ...
            if hasattr(order, 'parent') and order.parent is not None:
                ...
                if parent_ref in self.order_dict:
                    ...
                    if parent_status == order.Completed:
                        logging.warning(f"Child order {order.ref} failed for {symbol} with completed parent. Creating protection.")
            ...
            logging.warning(f'Order {symbol} failed with status: {order.getstatusname()}')
    ...
    def next(self):
        ...
        self.live_trades = self.trading_data
        ...
        logging.info(f"Active positions from IB: {list(active_positions.keys())}")
        ...
        # Print currently owned stocks and their purchase dates using the more accurate active_positions
        if active_positions:
            print("\nCurrently owned stocks (from IB):")
            for symbol, pos_info in active_positions.items():
                print(f"Symbol: {symbol}, Size: {pos_info['size']}, Avg Cost: {pos_info['avg_cost']}")
        ...
        for data in self.datas:
            ...
            # Use active_positions instead of getposition for more accurate data
            ib_position_size = active_positions.get(symbol, {}).get('size', 0)
            backtrader_position_size = self.getposition(data).size
            
            # Log any discrepancies for debugging
            if ib_position_size != backtrader_position_size:
                logging.warning(f"Position size discrepancy for {symbol}: IB reports {ib_position_size}, Backtrader reports {backtrader_position_size}")
            
            # Always use IB's position data as the source of truth
            actual_position_size = ib_position_size
            
            logging.info(f"{symbol} - IB Position size: {actual_position_size}, Closed flag: {self.position_closed[symbol]}")
    
            # Evaluate positions based on IB's data, not Backtrader's internal tracking
            if actual_position_size > 0 and not self.position_closed[symbol]:
                avg_cost = active_positions.get(symbol, {}).get('avg_cost', 0)
                logging.info(f"{symbol} - Current position size: {actual_position_size} at average price: {avg_cost}")
                
                # Pass position data to evaluate_sell_conditions
                self.evaluate_sell_conditions(data, current_date, actual_position_size, avg_cost)
                
            elif actual_position_size == 0 and self.position_closed[symbol]:
                self.position_closed[symbol] = False
                logging.info(f"{symbol} - Position closed and flag reset")
                
            elif actual_position_size == 0 and not self.position_closed[symbol]:
                logging.info(f'{symbol}: No position flag detected fresh check.')
                print(f'{symbol}: No position. Evaluating buy signals for data.')
                
                # Use total active positions for position count
                if len(active_positions) >= self.params.max_positions:
                ...
                logging.error(f'{symbol}: ERROR - We have shorted this stock! Current position size: {actual_position_size}')
    ...
    def handle_bracket_failure(self, symbol, size, avg_price):
        ...
        try:
            ...
            if data is None:
                logging.error(f"Cannot create protection orders for {symbol}: data feed not found")
                return

            # Calculate stop and take-profit prices
            stop_price = avg_price * (1 - self.params.stop_loss_percent/100)
            take_profit_price = avg_price * (1 + self.params.take_profit_multiplier)
            ...
            logging.info(f"Created standalone protection orders for {symbol}: "
                        f"Trailing stop at 3.0%, Take-profit at {take_profit_price}")

            return trailing_stop, take_profit

        except Exception as e:
            logging.error(f"Error creating protection orders for {symbol}: {e}")
            logging.error(traceback.format_exc())
            return None, None
    ...
    def synchronize_positions(self):
        ...
        """
        try:
            # Get all actual positions from IB
            ib_positions = {}
            ...
            for d in self.datas:
                symbol = d._name
                internal_size = self.getposition(d).size
                
                # Case 1: IB has position but we don't track it internally
                if symbol in ib_positions and internal_size == 0:
                    logging.info(f"Position synchronization: Found untracked position in {symbol}. "
                                f"IB has {ib_positions[symbol]['size']} shares, our tracking shows 0.")
                    
                    # Update trading data to reflect actual position
                    df = read_trading_data(is_live=True)
                    if symbol in df['Symbol'].values:
                        # Update existing row
                        df.loc[df['Symbol'] == symbol, 'IsCurrentlyBought'] = True
                        df.loc[df['Symbol'] == symbol, 'PositionSize'] = ib_positions[symbol]['size']
                        df.loc[df['Symbol'] == symbol, 'LastBuySignalPrice'] = ib_positions[symbol]['avg_cost']
                        df.loc[df['Symbol'] == symbol, 'LastBuySignalDate'] = pd.Timestamp.now()
                        write_trading_data(df, is_live=True)
                        
                        # Reset position closed flag
                        self.position_closed[symbol] = False
                        logging.info(f"Updated trading data for {symbol} to reflect actual IB position.")
                
                # Case 2: We track position internally but IB doesn't have it
                elif symbol not in ib_positions and internal_size > 0:
                    logging.warning(f"Position synchronization: Phantom position in {symbol}. "
                                  f"Our tracking shows {internal_size} shares, but IB has 0.")
                    ...
                    if symbol in df['Symbol'].values:
                        ...
                        write_trading_data(df, is_live=True)
                        logging.info(f"Updated trading data for {symbol} to remove phantom position.")
                
                # Case 3: Position size mismatch
                elif symbol in ib_positions and internal_size != ib_positions[symbol]['size']:
                    logging.warning(f"Position size mismatch for {symbol}: "
                                  f"IB has {ib_positions[symbol]['size']} shares, our tracking shows {internal_size}.")
                    ...
                    if symbol in df['Symbol'].values:
                        df.loc[df['Symbol'] == symbol, 'PositionSize'] = ib_positions[symbol]['size']
                        write_trading_data(df, is_live=True)
                        logging.info(f"Updated trading data for {symbol} with correct position size.")
            
            # Log total position count
            logging.info(f"Position synchronization complete. Active positions: {len(ib_positions)}")
            return ib_positions
            
        except Exception as e:
            logging.error(f"Error in position synchronization: {e}")
            logging.error(traceback.format_exc())
            return {}
